...
function handleSkillsCommand(args, rootDir) {
  ...
  if (subcommand === 'install') {
    ...
    const legacyArgs = ['--skills', skillName];
    if (flags.output) {
      legacyArgs.push('--output', flags.output);
    }
    if (flags.name) {
      legacyArgs.push('--name', flags.name);
    }
    if (flags.repo) {
      legacyArgs.push('--repo', flags.repo);
    }
    return runLegacy({ args: legacyArgs, rootDir });
  }

  throw new Error(`Error: unknown skills subcommand "${subcommand}"`);
}

function handleAuditCommand() {
  return;
}

function handleDocsCommand(args) {
  const subcommand = args[0];
  if (subcommand === 'suggest' || subcommand === 'write') {
    return;
  }
  throw new Error(`Error: unknown docs subcommand "${subcommand || ''}"`);
}

function runSubcommand(args, rootDir) {
