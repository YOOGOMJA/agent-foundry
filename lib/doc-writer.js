...
function ensureInsideRoot(rootDir, relativePath, context) {
...
function resolveTemplatePath(projectRoot, toolRoot, mapping) {
  const projectTemplatePath = ensureInsideRoot(projectRoot, mapping.source, 'project template path');
  if (fs.existsSync(projectTemplatePath)) {
    return projectTemplatePath;
  }

  const toolTemplatePath = ensureInsideRoot(toolRoot, mapping.source, 'tool template path');
...
function writeTarget(projectRoot, destination, content) {
  const targetPath = ensureInsideRoot(projectRoot, destination, 'doc destination path');
...
function normalizeDocMapping(entry) {
  ...
  const destination = normalizeSafeRelativePath(entry.destination);
